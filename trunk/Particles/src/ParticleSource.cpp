...
ParticleSource::ParticleSource()	// constructor, set default values
{
	...
	ptGravityAcceleration = Point(0, -9.8f, 0) ;
...
void ParticleSource::SetSpawner(Spawner *pSetSpawner)
{
	pSpawner = pSetSpawner ;
}

void ParticleSource::SetTexture(const char *szFilename)	// set texture for particles
{
...
void ParticleSource::SpawnParticles(int iNumSpawn)	// spawn specified number of particles
{
	...
	} else
	{
		...
		for (int iTrav = 0 ; iTrav < iNumSpawn ; iTrav ++)
		{
			...
			pNew->dLife = dInitialLife + dLifeDeviation * RandomPercent() - dLifeDeviation / 2.0 ;
...
void BillboardSpherical(Point ptWorldPos, Point ptCam)	// billboard calculation
{
	float angleCosine;
...
// be bigger than 1 due to lack of precision
	if ((angleCosine < 0.99990) && (angleCosine > -0.9999))
		glRotatef(acos(angleCosine)*180.0f/3.14f, ptUpAux.dVal[0], ptUpAux.dVal[1], ptUpAux.dVal[2]);	
...
bool ParticleSource::MoveParticle(Particle *pParticle, float fTime, LinkedList<Model> *pCollisionModel)	// update and move particle with collision testing
{
	Point ptAcceleration ;
	pParticle->dLife += pParticle->dLifeIncrimenter ;
	if (pParticle->dLife <= 0)
	{
		return 1 ;
	} else
	{
		...
		if (pCollisionModel != 0) CheckCollision(pParticle, pCollisionModel) ;
		//			pParticle->ptLocation = pParticle->ptLocation + pParticle->ptVelocity * fTime + pParticle->ptAcceleration * fTime * fTime ;
...
void ParticleSource::MoveParticles(float fTime, LinkedList<Model> *pCollisionModel)
{
	LinkedListNode<Particle> *pTravParticles = Particles.pHead ;
	...
	while (pTravParticles != 0)
	{
		if (MoveParticle(pTravParticles->pValue, fTime, pCollisionModel))
		{
...
void ParticleSource::MoveDisplayParticles(float fTime, LinkedList<Model> *pCollisionModel, Point ptCameraLocation)	// move and display particles
{
	glEnable(GL_BLEND) ;
	LinkedListNode<Particle> *pTravParticles = Particles.pHead ;
	LinkedListNode<Particle> *pTempParticle ;
...
// change to intersection calc
void ParticleSource::CheckCollision(Particle *pCheck, LinkedList<Model> *pModels) // detect and handle collision
{
/*	LinkedListNode<Model> *pTrav = pModels->pHead ;
	LinkedListNode<ModelPolygon> *pTravPolygons ;
	while (pTrav != 0)
	{
		if (pCheck->ptLocation.dVal[0] >= pTrav->pValue->ptModelBBNegatives.dVal[0] - 0.5 && pCheck->ptLocation.dVal[0] <= pTrav->pValue->ptModelBBPositives.dVal[0] &&
			pCheck->ptLocation.dVal[1] >= pTrav->pValue->ptModelBBNegatives.dVal[1] - 0.5 && pCheck->ptLocation.dVal[1] <= pTrav->pValue->ptModelBBPositives.dVal[1] &&
			pCheck->ptLocation.dVal[2] >= pTrav->pValue->ptModelBBNegatives.dVal[2] - 0.5 && pCheck->ptLocation.dVal[2] <= pTrav->pValue->ptModelBBPositives.dVal[2])
		{
			...
			while (pTravPolygons != 0)
			{
					...
					pCheck->ptLocation.dVal[2] >= pTravPolygons->pValue->ptBBNegatives.dVal[2] - 0.5 && pCheck->ptLocation.dVal[2] <= pTravPolygons->pValue->ptBBPositives.dVal[2])
				{
					...
					pCheck->ptVelocity = pCheck->ptVelocity + (ptMagnitude * (pTravPolygons->pValue->ptNormal * 1.2f)) ;
					return ;
