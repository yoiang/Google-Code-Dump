...
const Point Point::operator =(const Point &Assign)
{
	...
	if (&Assign == this)
	{
		ptTemp.dVal[0] = dVal[0] ;
		ptTemp.dVal[1] = dVal[1] ;
		ptTemp.dVal[2] = dVal[2] ;
	} else
	{		
...
{
	Point ptTemp ;
	ptTemp.dVal[0] = ptInitial.dVal[0] + ptDeviance.dVal[0] * RandomPercent() - ptDeviance.dVal[0] / 2.0f ;
	ptTemp.dVal[1] = ptInitial.dVal[1] + ptDeviance.dVal[1] * RandomPercent() - ptDeviance.dVal[1] / 2.0f ;
	ptTemp.dVal[2] = ptInitial.dVal[2] + ptDeviance.dVal[2] * RandomPercent() - ptDeviance.dVal[2] / 2.0f ;
	return ptTemp ;
}
